...
def do_nslookup(argv, input_str=None, outputfile=sys.stdout,
    ...
    modaction             = ModularAction(json.dumps(modaction_payload), logger, action_name="nslookup")
        ...
        for host in hosts:
            ...
            if re.match(host_validation, host):
                ...
                try:
                    ...
                    new_result['_raw'] = nslookup.communicate()[0]
                except Exception:
                    signature = 'Exception when executing nslookup command'
                    handleError(signature)
                    return
...
if __name__ == '__main__':
    logger = ModularAction.setup_logger('nslookup_modworkflow')

    do_nslookup(sys.argv, logger=logger)
