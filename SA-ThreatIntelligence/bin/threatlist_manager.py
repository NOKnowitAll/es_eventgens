...
class ThreatlistManagerModularInput(LookupModularInput):
    ...
    def __init__(self):
        ...
        self._logger = setup_logger(name='threatlist_manager', level=logging.INFO)
    ...
    def collect_stanzas(self, alt_modinput_name=None):
        ...
        to_ignore = ['action_links',
                     ...
                     'metadata',
                     'model_fields']
        stanzas = SplunkThreatlistInput.search('target!="threatlist"', sessionKey=self._input_config.session_key)
    ...
    def run_threads(self, files_by_category, last_run):
        ...
        merge_required.update(self.detect_changes(files_by_category, last_run))
        ...
        if merge_required:
            for target in merge_required:
                if target in self._valid_targets:
                    self._logger.info('status="merging" category=%s', target)
                    ...
                    self._logger.error('status="Invalid output target specified" target=%s', target)

            for t in threads:
                # Block until all threads complete.
                t.join()
            
            self._logger.info('status="merging complete" category=%s', target)            
            all_completed = all([event.is_set() for event in events])

        else:
            self._logger.info('status="no merging required"')
            all_completed = True
