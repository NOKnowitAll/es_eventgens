...
class NotableEventSuppression:
    ...
    def __init__(self, id, enabled, description=None, search=None):        
        ...
        if search is None:
            ...
            sourceMatch = NotableEventSuppression.sourceRE.findall(self.search)
        ...
        startMatch = NotableEventSuppression.startRE.findall(self.search)
        if len(startMatch) > 0:
            self.start = startMatch[0]
            
        else:
            logger.warn("Could not determine start time of suppression '%s' from search '%s'" % (id, self.search))
                    
        ## End Time
        self.end = ''
        
        endMatch = NotableEventSuppression.endRE.findall(self.search)
        if len(endMatch) > 0:
            self.end = endMatch[0]
            
        else:
            logger.warn("Could not determine expiration time of suppression '%s' from search '%s'" % (id, self.search))
    ...
    def __populate_entity__(self, entity, new=False, namespace=None, owner=None):
        ...
        entity["disabled"] = NotableEventSuppression.bool_to_int(not self.enabled)
    ...
    def set_suppression(id, enable, session_key=None):
        ...
        entity['disabled'] = NotableEventSuppression.bool_to_int(not enable)
    ...
    def bool_to_int(value):
        """
        Convert the given boolean value to a boolean to an int.
        """
