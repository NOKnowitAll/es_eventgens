    ...
    for nessusFile in os.listdir(options.srcdir):
        ...
        if nessusFile.endswith('.nessus'):
            ...
            with open(splunkFile, 'w') as splunkFH:
                _LOGGER.debug("Creating temporary Splunk Nessus file: %s", splunkFile)
            ...
            if os.path.isfile(splunkFile):
                ...
                try:
                    ...
                    _LOGGER.debug("Moving Splunk Nessus file to spool file: %s", os.path.join(options.tgtdir, nessusFile))

                    # Remove the original file.
                    try:
                        os.remove(nessusFilePath)
                    except Exception as detail:
                        _LOGGER.error('Could not delete original file from Nessus spool directory: %s' + str(detail))

                except Exception as detail:
                    _LOGGER.error('Could not move file from Nessus spool to Splunk spool: %s', str(detail))

            else:
                _LOGGER.error('Could not create temporary file for processing Nessus input: %s', splunkFile)
