...
class WhoisHttpOptions(WhoisOptions):
    ...
    def set_options(self, *args, **kwargs):
        ...
        valid_keys = ['api_host', 'api_user', 'app', 'debug', 'format', 'owner', 'proxy_port', 'proxy_server', 'proxy_user']
        for k in valid_keys:
...
class WhoisDomaintools(WhoisBase, WhoisHttpOptions, WhoisHttpHandler):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not getattr(self, '_api_host', False):
            # Fall back to using freedomaintools.com if host not provided.
            setattr(self, '_api_host', 'freeapi.domaintools.com')
    ...
    def prepare_query(self, query):
        ...
        if self._api_host is None:
            raise Exception('API host could not be retrieved.')

        self._query = None
        self._url = 'http://{0}'.format(self._api_host)
    ...
    def format(self, query, resolved_query, data):
        ...
        try:
            ...
            msg = "Unexpected exception when parsing query."
