...
class NetworkModularInput(ModularInput):
    ...
    def run(self, stanzas):
        ...
        try:
            if hasattr(self, '_alt_session_key'):
            ...
            if len(filenames) > 0:
                ...
                for filename in [f for f in filenames if f.endswith('.csv')]:
                    try:
                        ...
                        logger.error('Error opening checkpoint file: %s', os.path.join(self._input_config.checkpoint_dir, filename))
                        continue
                    ...
                    for count, query in enumerate(queries):
                        ...
                        if data:
                            ...
                            # The format() method may return None if Unicode or 
                            # other bad input was received.
                            # In this case, log the position of the error.
                            logger.error('ERROR: WHOIS query handler failed on a domain (filename=%s, line=%d', checkpointed_file.name, count+1)
